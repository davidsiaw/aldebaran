	...
	computer(map.CreateEntity())
{
	...
	TestSceneMap& theMap = map;
	map.ForeachTile([&](int x, int y, const TestSceneMap::entity_type &tile) {
		if (x % 2 != 0 && y % 2 != 0)
		{
...
void TestScene::Update(const InputState& inputs, Uint32 now)
{
	...
	RemoveWhere<TestSceneMap::entity_type>(&overlappingBombs, [&](TestSceneMap::entity_type bomb)->bool
	{
	...
	RemoveAndProcessWhere<bombInfoPair>(&bombs, 
		...
		[&](bombInfoPair bip)
		{
			...
			std::tr1::function<bool(TestScene::ExplosionInfo)> isVulnerable = [&](TestScene::ExplosionInfo ex)->bool
			{
				...
				return 
					(status == TestSceneMap::OCCUPIED && aMap.GetEntity(ex.x, ex.y)->userdata != BLOCKID) ||
					(status == TestSceneMap::FREE);
			};
...
{
	map.ForeachTile([&](int x, int y, const TestSceneMap::entity_type &ntt)
	{
		SDL_Rect r;
		r.w = 64;
		r.h = 64;
	...
	map.ForeachEntity([&](const TestSceneMap::entity_type &ntt)
	{
		SDL_Rect r;
		r.w = 64;
		r.h = 64;
