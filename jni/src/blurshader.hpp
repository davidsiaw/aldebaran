...
class BlurShader : public ShaderInterface
{
    GLuint shader;
    ...
    static GLuint SetUpShader()
    {
        ...
        STRINGIFY(
                  ...
                  void main()
                  {
                      ...
                      sum += texture2D(uTexture, vec2(vTexCoord.x, vTexCoord.y - 4.0*blurSize)) * 0.05;
                      sum += texture2D(uTexture, vec2(vTexCoord.x, vTexCoord.y - 3.0*blurSize)) * 0.09;
                      sum += texture2D(uTexture, vec2(vTexCoord.x, vTexCoord.y - 2.0*blurSize)) * 0.12;
                      sum += texture2D(uTexture, vec2(vTexCoord.x, vTexCoord.y - blurSize)) * 0.15;
