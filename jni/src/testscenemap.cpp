#include <iostream>
#include "testscenemap.hpp"
#include "printlog.hpp"

TestSceneMap::TestSceneMap(int w, int h) : map(boost::extents[w][h]), ids(0)
{
}
...
TestSceneMap::entity_type TestSceneMap::CreateEntity()
{
...
TestSceneMap::entity_type TestSceneMap::GetEntity(int x, int y)
{
	return map[x][y];
}

TestSceneMap::entity_type TestSceneMap::GetEntity(int id)
{
	return reg[id].lock();
}

bool TestSceneMap::TrySetEntity(const entity_type &ntt, int x, int y)
{
	if (!map[x][y])
	{
		map[x][y] = ntt;
...
TestSceneMap::entity_type TestSceneMap::RemoveEntity(int x, int y)
{
	auto ntt = map[x][y];
	map[x][y] = entity_type();
	return ntt;
...
TestSceneMap::PositionCheck TestSceneMap::CheckPosIsFree(int x, int y)
{
...
void TestSceneMap::Update(int timeSteps)
{
...
void TestSceneMap::ForeachTile(std::tr1::function<void(int,int, const std::tr1::shared_ptr<Entity> &)> func)
{
	typedef map_type::index index;
	index xMin = map.index_bases()[0];
...
void TestSceneMap::ForeachEntity(std::tr1::function<void(const std::tr1::shared_ptr<Entity> &)> func)
{
	BOOST_FOREACH(registry_type::value_type iter, reg)
	{
		auto ntt = iter.second.lock();
